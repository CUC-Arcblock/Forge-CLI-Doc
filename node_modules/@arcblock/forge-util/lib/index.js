...
const multibase = require('multibase');  // 进行各种Base编码的开源库
...
const hexToBytes = hex => {
  ...
  if (!isHex(hex)) {
    // 判断hex不是一个十六进制对应的字符串值，报错
    throw new Error(`Given value "${hex}" is not a valid hex string.`);
  }

  hex = hex.replace(/^0x/i, '');
  // js的string.replace()方法的第一个参数可以为正则表达式，第二个参数可以为函数或者字符串
  // 匹配从首开始的0x,  换为''
  hex = hex.length % 2 ? `0${hex}` : hex;
  // 将hex的长度通过补0转成2的倍数，因为2位可以转成一个Byte

  const bytes = [];
  for (let c = 0; c < hex.length; c += 2) {
    bytes.push(parseInt(hex.substr(c, 2), 16));
    //parseInt(string,radix):将字符串string以基数radix进行解析，返回解析后的整数值。
  }
